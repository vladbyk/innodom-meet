...
const Room = (data, exitUser) => {
    ...
    const createPeerConnection = useCallback((socketID, localStream, email) => {
        ...
        pc.ontrack = (e) => {
            ...
            console.log(tracks)
            if (len === 2) setUsers((oldUsers) => oldUsers.filter(user => user.id !== socketID))
            setUsers(oldUsers => {
    ...
    const screenSharing = () => {
        navigator.mediaDevices.getDisplayMedia({video: {displaySurface: "monitor", frameRate: 60}})
            .then((stream) => {
                console.log(pcs)
    ...
    return (<div>
            ...
            {isVideo ? <button onClick={() => {
                ...
                setVideo(false)
            }}>выкл video</button> : <button onClick={() => {
                const videoTracks = localVideo.current.srcObject.getVideoTracks()
                videoTracks.forEach((track) => {
                    track.enabled = true
                })
            ...
            {isDispVideo && <video muted width='400px' autoPlay ref={localDisplayVideo}></video>}
            {users.length > 0 && users.map((user, index) => (<Video key={index} stream={user.stream} user={user}/>))}
        </div>);
}
